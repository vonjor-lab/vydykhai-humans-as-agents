...
- **Сам проверяет готовность карты проекта.** При подключении и подходящем обновлении находит недостающие карты кода, модулей и контракты, назначает одну задачу на проверку существующей архитектуры и показывает ограничения. Изменение границ предлагает человеку до правок; независимая работа продолжается.
...
- **Сохраняет смысл по мере роста проекта.** Перед изменением AI проходит от цели к затронутым модулям, их входам, результатам и зависимым частям, читает актуальную документацию и только затем код. Изменение поведения и документации идет одной работой. Прежние решения, идеи, уроки и обещания вернуться находятся по связанным сущностям; если карта неполна или противоречива, система сначала проводит ограниченное исследование, а не угадывает и не строит функцию заново.
...
- **Preserves understanding as the project grows.** Before a change, AI follows the goal through touched modules, their inputs, outputs and dependent parts, reads current module documentation, and inspects owned code only when changing it. Ready modules connect through documented public interfaces and fixed releases, without rereading or patching their internals. Behavior and documentation change in the same piece of work. Prior decisions, ideas, lessons and promises to return are found through related entities; if the map is missing or contradictory, the system runs bounded discovery instead of guessing or rebuilding the capability from scratch.
...
- Every focused context receives only the applicable goal, boundary, entity route, module documentation, memory, verification, human checkpoint, and return route. It distinguishes consuming a released module from developing it: public contract and release for use, maintainer design and owned code for authorized change; exact multi-part obligations use a checked envelope so none can disappear during summarization. The task continues autonomously inside that contract and consults the orchestrator only at a real boundary.
...
The orchestrator records the plan in Project State so the team prepares one update rather than duplicate work. At the chosen window, one focused maintenance task prepares or reuses the update branch, runs `update` and `doctor`, opens or refreshes the PR, and returns evidence. The orchestrator coordinates that task and reports the short delta; it does not perform the installation. Unfinished workers adopt at safe checkpoints or before resume; paused work stays paused, and matching files alone do not prove that a worker has read the instructions. After authorized merge, the active orchestrator verifies its own working directory: accepted project revision, installed version and source revision, current schemas, integrity through its own `doctor`, and reread updated core. A temporary update or merged-source worktree cannot certify the active context. If the active context cannot move cleanly, route bounded repair or confirmed rotation instead of changing the title or Project State early. No updater overwrites managed-file conflicts, changes rules silently mid-operation, or merges outside project policy. A major migration or safety-affecting update is raised explicitly; an ordinary compatible update does not force rotation. If upstream is unavailable, the check remains pending without blocking otherwise safe work.
