...
- Instruct every launched execution context to start implementation, resolve ordinary failures autonomously, send compact `CONSULT` only at an undeclared boundary, never change its reasoning profile mechanically, run `$accept-work`, record each executable Memory Brief item as applied/missed/contradicted/not exercised, and publish Return Sync with a unique receipt id plus `NO_MEMORY_DELTA` or compact candidates only at a named human checkpoint, irreducible blocker, or terminal result. The orchestrator reads back actual title/link, role/profile, active start, and Return Sync route before declaring launch. Publish the task's current contract and tracker fields in the same approved operation. Return approved work to the Framework Orchestrator; do not implement or run orchestration/alignment workflows there.
