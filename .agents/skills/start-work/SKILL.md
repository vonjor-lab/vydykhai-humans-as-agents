...
- Build autonomous role-`EXECUTION` tasks with the minimum contract: nearest goal/DOD, scope/out of scope, outcome owner and dependency/recipient boundary, freshness/Baseline, accepted mechanism plus 1-3 distilled memory items, product loop or enabler `Unlocks / Still missing / next product slice`, authority/safety envelope, checkpoint, material burn, verification, and compact consult/return triggers. Do not pass full Project State, transcripts, memory views, task map, or orchestration deliberation.
...
- Instruct every launched context to start implementation, resolve ordinary failures autonomously, send compact `CONSULT` only at an undeclared boundary, run `$accept-work`, and publish Return Sync only at a named human checkpoint, irreducible blocker, or terminal result. Return approved work to the Framework Orchestrator; do not implement or run orchestration/alignment workflows there.
