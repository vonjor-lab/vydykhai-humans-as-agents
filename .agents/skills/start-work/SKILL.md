...
- Build autonomous tasks with the minimum contract: goal/DOD, scope, outcome owner and dependency/recipient boundary, freshness/Baseline, accepted mechanism plus 1-3 applicable invariants, product loop or enabler `Unlocks / Still missing / next product slice`, checkpoint, burn/expansion, verification, and `Consult when / Return to` plus terminal return triggers.
