...
- Check context sufficiency for every task, but invoke opt-in `PREPARATION` only for missing/stale/scattered context. It retrieves graph meaning, Module Contracts and indexed code; it does not decide new architecture. Review role applicability and gaps, then pass the approved packet to a fresh executor without search history. Keep preparation-only permissions out of executor instructions. Reuse current packets and return to the accepted route if cheap retrieval is unproven; a successful packet handoff continues authorized work, not an ACK-only stop.
