...
Shape large work before implementation begins. The first-action receipt proves execution has started; it does not add a pause or approval between already authorized steps. At dispatch/material resume apply Execution Readiness in the actual worker, plus `docs/workflows/task-context-handoff-template.md#return-authorization`: read-only sources do not imply writable reports or permitted delivery.
...
- After activation, new implementation tasks touching durable modules with prior obligations or acceptance behavior use `retained-progress-v1` and `docs/workflows/context-preparation.md` by default. The orchestrator reviews sources/plan; the assigned worker reads and acknowledges actual context before `context-run` execution. Missing critical access stays scoped pending/LIMITED. Scoped research may resolve it without already having that complete implementation packet (`context-routing.md`); no preparation-of-preparation loop. Existing workers enter at safe rebrief/readback, never by silent installation inheritance.
- Check context sufficiency for every task, but invoke opt-in `PREPARATION` only for missing/stale/scattered context. The orchestrator assigns and reviews retrieval of graph meaning, Module Contracts and indexed code; the preparer does not decide new architecture. Follow `context-routing.md`: initial approved handoff to a fresh executor without search history; later executor `CONSULT` -> orchestrator -> preparer supplement -> orchestrator review -> same executor with retained progress. Keep preparation-only permissions out of executor instructions. Reuse current packets and return to the accepted route if cheap retrieval is unproven; successful handoff continues authorized work, not an ACK-only stop.
...
- Resolve the vertical goal-to-entity spine and reverse consumer routes before filtering. For every touched durable module or capability, supply its current project Module Contract shaped by `docs/workflows/module-contract-template.md` and the exact public-use or owned implementation/test boundary. Classify consume/change/create under the Module Contract workflow: consumers read public contracts and fixed releases, not producer internals; only authorized changes add maintainer design and owned code/tests. Agree new/changed boundaries with the person before mutation; a missing or contradictory link is `MEMORY_COVERAGE_GAP`, not implicit permission to rediscover or replace behavior.
