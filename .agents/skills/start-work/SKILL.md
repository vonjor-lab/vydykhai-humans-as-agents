...
- Derive a Touch Set from outcomes, entities, actors/surfaces, contracts/authorities, and data/operational realms. Match it to the Project Memory Graph, accepted/rejected task lineage, durable evidence, and active work; retain that project-wide reasoning in the orchestrator and give the task only the smallest complete Memory Brief of up to seven nodes or raise `MEMORY_COVERAGE_GAP`.
