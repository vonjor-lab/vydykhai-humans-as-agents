...
- Explain the working model plainly: the personal orchestrator decides what/why/when/who and what changed; separate research/lab/task contexts decide how to execute; ordinary continue uses a lightweight hot path; tasks solve local failures and return only at declared triggers; acceptance and merge stay in the task. The orchestrator recalls relevant decisions before work and learns reusable deltas afterward; people do not curate memory manually.
