...
- Name the active control context `[ORCHESTRATOR] <project> — Vydykhai <version>`, verify the actual title, and pin or foreground exactly that one context when supported. Use the project's working language for outcome text and fixed metadata tags for navigation.
