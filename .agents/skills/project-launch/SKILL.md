...
- Resolve capability-and-cost profiles: flagship maximum `ORCHESTRATOR` and deep bounded `DISCOVERY`; proven efficient `EXECUTION` and opt-in retrieval `PREPARATION`. Map to `Ultra / XHigh / Low` where available. Record actual model/effort, capability evidence, date/source and explicit fallback; no universal vendor id or silent downgrade. Missing preparation proof keeps the established route usable.
...
- For an existing project with meaningful history, route one focused side-by-side reconstruction from the accepted brief or earliest reliable baseline without blocking safe current work or mutating accepted shared state. In that Candidate, build a Source Coverage Ledger through the current watermark across accessible human dialogue, meetings, tasks, participant packets, PRs, code and accepted artifacts; process each bounded source range once. Build the entity spine and create or repair Module Contracts before attaching atomic current meaning, let later evidence explicitly supersede earlier meaning, and retain gaps rather than guessing. Prove current, next, cross-domain, prior-miss and sufficient-context queries through task application and update replay before cutover. The orchestrator only routes and later integrates accepted shared memory; it does not author product documentation.
- Create atomic Project State v2 with activation, external Project Guard registration, `Human attention: NONE`, DOD Control Line, Governor receipt, Execution Leases, Pending Return Inbox, detour/recall gates, Shared Sync, participants, orchestrator, and tracker projection. Record each current fact once in its owning control section; activation evidence is immutable and tracker/legacy summaries are non-authoritative views. Render and validate one complete Candidate before the first write, then read back the exact SHA-256 and `control-check`; restore the accepted body on mismatch. Use Project Memory Graph v4 with a stable entity spine, atomic nodes, Source Coverage Ledger, Module Contracts, pending events, and `CURRENT / NEXT / CROSS_DOMAIN / PRIOR_MISS` probes, advancing it only for semantic memory deltas. Read compatible v3 during migration; require visible delta, source-to-task proof, human-confirmed cutover, and passing `control-check`.
...
- Configure the marked task Return Sync as durable tracker outbox first and notification by its single accepted owner second (native by default, never parallel Guard delivery), then require the orchestrator's paired marked Return Route receipt. Verify one id through `WRITTEN / SENT / RECEIVED / CONSUMED / ROUTED` on the first real dispatch and a subsequent scheduled Guard `NOOP`, without human polling. Give Project Guard the durable route and proven adapters for the selected mode; record the scope-freshness interval, default seven days.
