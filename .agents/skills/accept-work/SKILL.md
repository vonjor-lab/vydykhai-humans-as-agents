...
- At a named human checkpoint, irreducible blocker, or terminal result, publish one Return Sync with learning/approach evidence; `Memory candidates: NO_MEMORY_DELTA`, `task-local only`, or compact `ADD / REFINE / SUPERSEDE / RETIRE / CONFLICT` candidates; and an `Artifact disposition` for the context, PR, branch, worktree, runtime, and monitor. Finish or clean only artifacts proven safe to remove, with unique work incorporated or preserved; mark ambiguous or useful stale work for `WAITING`, `SALVAGE`, or `RETIRE` with evidence/re-entry instead of auto-deleting it. Do not edit the shared graph or emit routine progress returns. The orchestrator integrates reusable lessons and performs project-wide follow-through without repeating cleanup. Cross-person delivery remains incomplete until recipient proof.
