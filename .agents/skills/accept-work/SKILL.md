...
- At a named human checkpoint, irreducible blocker, or terminal result, write one complete Return Sync to the durable task/tracker outbox, then send the same id as a native wakeup. Include learning evidence, memory candidates or `NO_MEMORY_DELTA`, and `Artifact disposition`. Report only through `SENT`; the orchestrator owns receipt, routing, memory integration, and lease closure. Do not emit routine returns. Cross-person delivery remains incomplete until recipient proof.
