...
- Reconstruct the Accepted Baseline, Candidate, executable Memory Brief, its goal-to-evidence Context Route, supplied current Module Contracts, and task-local approach lineage using source precedence. When an atomic envelope is present, load `docs/workflows/memory-brief-envelope.md` and validate its application receipt before accepting any claimed applied ids. Compare actual scope and affected consumers with the supplied constraints, Module Contracts, current code and tests; apply `docs/workflows/context-route.md` without rebuilding unrelated memory. Do not let an old issue, plan, or Rejected Candidate override a later proven state, direct human correction, or targeted orchestrator patch.
...
- For declared `retained-progress-v1`, run `context-run accept` under `docs/workflows/context-route.md`: retained plus promised new observations must bind to actual Candidate/input hashes. Return local source deltas for shared integration. Reference `VERIFIED` is evidence, not human acceptance, native adoption or parent closure.
...
- For Lab Mode, verify its decision, one-variable contract, proof, Learning Delta, production transfer, tests, and risk-based real-flow verification before product acceptance. Check the agreed entry and mechanism: a fixture or disabled AI limits proof to the exercised boundary, never justifies replacing an untested producer. Agree departures from promised test conditions before using them as acceptance evidence.
...
- Treat missing or inconclusive current-code smoke as `NEEDS_FIXES` and continue task-owned correction/verification within scope and burn. Use `BLOCKED` only for an evidenced unresolved boundary with a resume condition, not merely because an internal test failed.
...
- At a named readiness result, human checkpoint, irreducible blocker, or terminal result, write one complete marked Return Sync to the durable task/tracker outbox, then use its single accepted notification owner under Return Authorization (native by default, never parallel Guard delivery). An Action Receipt never substitutes for Return Sync. Include learning evidence, memory candidates or `NO_MEMORY_DELTA`, `Documentation impact: NONE` or exact Module Contract files/sections updated in the same Candidate plus graph-route candidates, and `Artifact disposition`. Report only through `SENT`; the orchestrator owns receipt, routing, the paired marked Return Route receipt, shared-memory integration, and lease closure, but never edits product code or module documentation. Native final text and task/thread reads are non-authoritative and may be empty. Do not emit routine returns. Cross-person delivery remains incomplete until recipient proof.
