...
- If the Candidate is rejected or the human says «do it differently», record Before/Now/Why/Keep/Drop/touch keys/source and return a `DECISION` candidate when reusable. Build any successor from the Accepted Baseline plus applicable Memory Brief while carrying forward proven changes and lessons.
...
- At a named human checkpoint, irreducible blocker, or terminal result, publish one Return Sync with learning/approach evidence plus `Memory candidates: NO_MEMORY_DELTA`, `task-local only`, or compact `ADD / REFINE / SUPERSEDE / RETIRE / CONFLICT` candidates. Do not edit the shared graph or emit routine progress returns. The orchestrator integrates reusable lessons and performs project-wide follow-through. Cross-person delivery remains incomplete until recipient proof.
