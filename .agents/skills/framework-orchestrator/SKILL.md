...
- Before a cold-path brief, re-brief, dispatch, consultation decision, sequence decision, parent acceptance, milestone, or rotation, derive a Touch Set and retrieve the smallest complete Memory Brief from the Project Memory Graph, normally three to seven nodes and fewer when fewer apply. Include sources or raise `MEMORY_COVERAGE_GAP`; never send the full graph to a task.
- Integrate reusable task, meeting, acceptance, and human-correction candidates with `ADD`, `REFINE`, `SUPERSEDE`, `RETIRE`, `CONFLICT`, or `NO_CHANGE`. Re-read the graph watermark and unseen events before each body write; recompute on concurrent change, merge semantic duplicates, preserve evidence/id mapping, and never copy secret values.
...
- Dispatch role `EXECUTION` on the efficient profile only when Low-ready: one outcome and first action, no unresolved solution choice, explicit boundaries, objective acceptance, current inputs/access/environment, and compact material consult triggers. Add the nearest DOD, owner/dependency boundary, freshness/Baseline, accepted mechanism, Memory Brief, safety, burn, and verification. Update the issue's current contract, formal links, Project State, and tracker projection together. Do not pass raw Project State, transcripts, full memory, task map, or orchestration deliberation.
