...
- The orchestrator decides what, why, when, and who, and maintains the DOD Control Line, sequence, alignment, dispatch, human requests, shared memory, parent closure, and next-best-action. A lightweight Governor Check independently tests this control loop from durable state and routes bounded repair or orchestrator rotation; it never performs project work.
...
Connect and prove the repo, participants, inputs, current-DOD operations, source of truth, privacy rules, compass, atomic Project State/graph, DOD Control Line, and Governor baseline. Register each active orchestrator and publish the Project Activation Receipt before first dispatch.
...
Governor Check runs at activation, dispatch/material resume, Return Sync consumption, human correction or detour, milestone/update, first active event after 24 hours, two unchecked compactions, two same-class control failures, DOD reconstruction failure, ambiguous side-effect outcome, or chat archaeology. Deterministic `control-check` runs first; a fresh maximum-profile evaluator is used only for anomaly or semantic health. It reads durable sources independently and returns `HEALTHY`, bounded `REPAIR`, or confirmed-path `ROTATE` without doing project work. A broader Health Review remains for several slices, stalled DOD burn, recurring expansion/tax, owner dropout, memory quality, and work hygiene.
...
- Keep one DOD Control Line, one unresolved Execution Lease per increment, durable-outbox-first Return Sync, and explicit detour return gates. Governor independently verifies these controls and routes repair or confirmed rotation; people never poll tasks or remember postponed work for the system.
