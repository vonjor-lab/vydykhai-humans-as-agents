...
- Project Memory Graph v4 links a stable goal-to-module entity map with atomic requirements, decisions, lessons, commitments, ideas, safe operational pointers, source coverage, and current Module Contracts. Compatible v3 remains readable until a separately proven cutover. Ordinary framework updates include [activation](workflows/framework-activation.md); for new qualifying tasks, the orchestrator [prepares reviewed sources](workflows/context-preparation.md); the worker reads and acknowledges actual context, then verifies both retained and new behavior through [Context Route](workflows/context-route.md#bounded-reference-caller). Installing commands does not activate older tasks. The common path needs no hooks; optional native gating covers one trusted declared command. Legacy artifacts remain readable; complete history, fresh-worker interpretation, desktop inheritance and product reliability are not established by bounded CLI proof. Guard gains no semantic timer work.
- The orchestrator decides what, why, when, and who, and maintains the DOD Control Line, sequence, alignment, dispatch, human requests, shared memory, parent closure, and next-best-action. It releases the control turn when ready management work has an observable productive handoff or a concrete wait, relying on durable returns instead of polling. Service input does not replace that obligation. A project-owned external Project Guard survives the orchestrator context: events and a schedule run deterministic checks, while a fresh maximum-profile Governor evaluates only anomalies and routes bounded repair or rotation. Neither performs project work. The flagship orchestrator may use bounded internal advisory agents to improve a control decision, but their analysis is disposable and cannot own a project result, create accepted project evidence, or replace a focused context.
...
3. `$project-launch` shapes an unclear goal when needed, then creates the Operating Brief, atomic Project State v2, Project Memory Graph v4, first DOD Control Line, Shared Sync Contract, tracker route, and safe operational pointers. It links existing current Module Contracts and routes any missing documentation or historical reconstruction to a focused Candidate-producing task. Project State changes become current only after a complete Candidate passes validation, one guarded publication, and exact hash/readback; mismatch restores the last accepted body.
...
- `ORCHESTRATOR`: current flagship at `Low` for compass, memory, routing, task design, consultation, integration, and next-best-action. An evidenced control failure still present after one targeted correction temporarily uses `High`; return to `Low` at a safe boundary after verified recovery. Follow the bounded recovery rules in `context-routing.md`.
- `DISCOVERY`: current flagship at `High` for a solution that is not yet defined: bounded research, product or architecture choices, unresolved UX/UI or visual direction, and experiment design. Cheap preparation may supply evidence, not replace this judgment.
...
- `PREPARATION`: opt-in bounded retrieval, mapped to `Low`, on a proven low-cost model. The orchestrator assigns and reviews it under `context-routing.md`: reuse sufficient context, otherwise initial source-backed handoff to a fresh executor; later gaps return through the orchestrator for a supplement to the same worker. No inherited search history, new design authority or executor-managed preparation.
Orchestration and unresolved Discovery retain the current flagship. Without comparable evidence, retain the accepted execution profile or record an explicit flagship fallback. These are environment mappings, not vendor requirements. Record actual model/effort, capability evidence, date/source and fallback in Project State; recheck at bootstrap, update, rotation, rejection/deprecation and active Health Review at least every seven days. Pass the selected profile explicitly; never silently substitute it. A resumed task keeps its accepted profile until a re-brief changes it. Use dated credit-rate coefficients as a planning proxy across preparation, coordination, execution, review and rescue; calibrate with attributable practical usage, never label that proxy measured subscription savings.
...
A human may explicitly override the recommendation. Record the reason, limits, and return condition. A command, prompt, plan, or send attempt is not a completed transition: its owner reads back `Trigger / Retrieved rule / Expected / Observed / Evidence / PASS|MISMATCH|UNVERIFIED|OUTCOME_UNKNOWN`. The orchestrator owns task-launch/resume, Return Sync, and memory-reflection/detour receipts; the acting context owns protected-access receipts; the task owns acceptance/live and side-effect receipts. Only `PASS` closes a transition. Routine control transitions use deterministic validation, one publication, and one readback. A Governor receipt is bound to its semantic anomaly and inspected evidence, not every later State revision; it never closes a changed anomaly or authorizes live work. A mismatch stops only that transition; an uncertain external outcome freezes replay until exact durable state is reconciled.
...
Every increment follows a [Context Route](workflows/context-route.md): goal/brief -> outcome/journey -> module/capability -> entities, contracts, data, artifacts and systems -> current meaning and commitments -> consumers -> accepted artifacts and verification. Completeness comes before context length; preserve material rationale and source qualifications. Graph v4 separates this stable entity map from atomic current meaning and links every durable module/capability to a current [Module Contract](workflows/module-contract-template.md). Structural validity is not semantic coverage or proof of live behavior. Existing projects process source ranges once from their accepted brief to the current watermark and build a v4 Candidate beside compatible v3 without interrupting unrelated work. New ideas must not delay the nearest DOD, useful ideas must not disappear, and meaningful intent, decisions, lessons, commitments, documentation and operational knowledge must survive people, tasks, and orchestrator changes. Project State is the atomic control snapshot; Project Memory Graph is reusable semantic memory and advances only for a memory delta. Raw history remains linked evidence; each task receives only a working capsule.
...
- A human request to remember and revisit something is a first-class `COMMITMENT`, not ordinary history. Attach it to affected entities and preserve its current meaning, source, aliases and trigger, applicability/timing, owner, return or closure condition, evidence, and any pending human question. If timing is unresolved, return the question at the first matching planning touch instead of choosing a horizon.
...
- Give outcomes, actors, journeys, modules, capabilities, entities, surfaces, contracts, data, artifacts, systems, and operations stable anchors and aliases. Connect them with `part-of / serves / produces / consumes / depends-on / stored-in / constrains`, traversed both ways. Keep one independently changeable meaning per node: `INVARIANT`, `REQUIREMENT`, current `DECISION`, failed-path `LESSON`, open `COMMITMENT`, future `IDEA`, or safe operational `POINTER`. A protected `POINTER` is executable only when current memory names its owner, protected reference without the value, environment and scope, allowed non-destructive route, last safe check with time/result/source, and expiry or re-entry condition.
...
At each cold-path brief, re-brief, dispatch, consultation or sequence decision, parent acceptance, milestone, and rotation, the orchestrator resolves exact anchors and aliases from the Touch Set, intersects matching open recall commitments and checkpoints before filtering dormant items, adds semantic candidates, traverses relevant typed relations until the applicable goal, contract, decision and consumer context is complete, and filters by source precedence, status, scope, applicability, and supersession. It returns all applicable meaning without a fixed node cap as executable items: `Because / Apply / Avoid / Verify / Source`; opaque ids alone are invalid. Raise `MEMORY_COVERAGE_GAP` when required meaning is unproven. When omission, merging or reordering would invalidate an indivisible sequence, matrix, safety gate or proven compression miss, carry only that portion through the compiler-derived [Executable Memory Brief](workflows/memory-brief-envelope.md) and require its bound application receipt; ordinary context remains prose. Compaction or migration is tested first with three or four cheap, ordinary, unhinted real-task queries. Any miss stops promotion before broad actor evaluation; targeted regression follows only for the exposed defect, then atomic shadow integration and human-confirmed cutover. Passing synthetic cases before this natural test is not semantic coverage. An incomplete protected pointer blocks only the affected action before secret re-request, historical reconstruction, or live mutation; repair it from linked durable evidence, rerun retrieval, and continue only after the complete safe pointer and current non-destructive check are proven. Re-read the graph watermark before integration, preserve evidence, and do not repeat retrieval on a hot-path continue.

For every cold-path task, traverse the entity spine in both directions: from the goal through the touched module to its inputs, outputs and dependent consumers, then back to applicable commitments and accepted evidence. Classify each touched module as consume/change/create under its Module Contract. Consume via public documentation and a fixed release without producer-source exploration; authorized development adds maintainer design and owned code/tests. Agree new/changed boundaries with the human, preserve accepted releases, and prove independent consumer connection before calling a module packaged. Module composition uses public boundaries, not internal-tree expansion. A missing, stale or contradictory route, contract, artifact or implementation boundary creates a scoped `MEMORY_COVERAGE_GAP` and bounded `DISCOVERY`; a complete current route proceeds directly to execution.
...
Research reduces uncertainty. Lab reduces execution cost. A task delivers accepted product or enabling work. [Context Routing](workflows/context-routing.md#fit-with-existing-work) separates factual Preparation from solution Discovery across all modes: research can start with explicit gaps; complete implementation context is its possible output, not a recursive entry gate. Reuse sufficient packets and keep required documentation inside the accepted Candidate.
A Discovery Decision Packet names the chosen approach, rejected options and lessons, affected entities/interfaces, evidence, risks and unresolved owner decisions. The orchestrator integrates it before execution. A bounded lead retains only named design/review obligations after that checkpoint, uses the same Return Sync and concrete waits, and requires verified whole-lease Guard coverage; otherwise use ordinary Discovery. No permanent role, private memory or new scheduler is added.
...
- Scope freshness, Accepted Baseline, accepted mechanism, complete applicable Memory Brief items, current Module Contracts and implementation boundaries, expected documentation impact, and unchanged compiler output plus application receipt only for declared atomic obligations;
...
### 4. Execute
The task context first checks actual cwd, source reads, permitted report storage and delivery under `workflows/task-context-handoff-template.md#execution-readiness`, then starts a safe observable action without repeating approved planning. Empty views or missing reports require exact-turn outcome reconciliation, not blind retry. It owns implementation, debugging, tests, fixes and durable checkpoints until a named human checkpoint, irreducible blocker or terminal result. It does not run project launch, shaping, alignment, orchestration or Governor workflows.
...
Do not mechanically escalate a struggling task through reasoning levels. First distinguish an implementation defect, a weak acceptance oracle, and missing solution work. Rebuild from the Accepted Baseline with learned evidence, re-brief, or launch bounded Discovery. High-consequence execution may still use the efficient profile only with explicit invariants, deterministic guards, and orchestrator review of evidence and project coherence before human merge or action approval; task acceptance stays in the task.

At a declared return trigger, including a readiness result or human checkpoint, the task uses the installed canonical writer to put one marked Return Sync with a unique receipt id and `NO_MEMORY_DELTA`, task-local evidence, or reusable candidates in the durable task/tracker outbox, then attempts to send the same id as a native wakeup. Status qualification is a separate detail, not invented slash grammar. The shared reader accepts only known legacy aliases/qualifiers and rejects mixed aliases, so old valid history remains usable without rewriting it. An Action Receipt may prove an action but never substitutes for terminal Return Sync. A native final, thread read, or delivery status is non-authoritative and may be empty even when the task journal contains a result. On every cold path and Governor Check, the orchestrator reconciles `WRITTEN -> SENT -> RECEIVED -> CONSUMED -> ROUTED` and appends the paired marked Return Route receipt. Project Guard compares those real producer/consumer records on the write event and active timer. Therefore lost native delivery cannot lose the result or require human polling. It verifies sender, evidence, DOD route, and next action before closing the lease. Cross-person delivery still requires exact artifact/revision, environment, reproducible safe data when relevant, recipient access, and the agreed check. A monitor is only fallback when neither normal route exists.

Guard adapters reuse the canonical outbox parser, refresh sources incrementally, and recheck the pending return and current recipient before delivery. Unrelated memory changes do not erase waiting work; incomplete routing does not close it. Prove adoption through installed event and schedule routes, not kit version alone (`workflows/project-guard.md`). Apply `workflows/task-context-handoff-template.md#return-authorization`: verify the current human source, recipient and permitted report data; preserve results as `WRITTEN` until transport succeeds. Guard or another outbox cannot bypass a security-denied disclosure as if it were a lost notification. Reuse applicable approvals unless scope or host policy requires more; framework receipts never grant host permissions. The first-action receipt proves start, not extra approval. Pending background checks block only the affected capability, not independent safe work.
...
The task reports `Documentation impact: NONE` or updates the affected Module Contracts in the same Candidate and names the exact files/sections plus graph-route candidates. The orchestrator verifies that accepted behavior and documentation agree, then integrates only reusable shared-memory meaning and routes before dependent dispatch. It does not edit product code or module documentation itself, and the task does not rewrite shared memory directly.

### 7. Review Health

Project Guard is an operation, not a permanent context. One project-owned runner outside the orchestrator invokes deterministic `guard-check` on material control events and an independent timer. In observation/recovery mode it compares marked Return Sync and Return Route records with fresh native activity. The runner keeps one incident ledger outside Project State; incident identity is the semantic condition set, not snapshot hash. Related changes settle into one check, routine State revisions and stale display views stay model-free, and Project Memory Graph is untouched unless memory meaning changed. Command wrappers preserve the checker's exit status. A live lock remains held; only a proven stale same-host lock is quarantined and rechecked after external outcome reconciliation, never used to authorize replay. Durable waiting work gets one idempotent wakeup; only a genuine anomaly or a proved unresolved wake starts a fresh maximum-profile Governor. `HEALTHY` closes that incident; `REPAIR` permits one transactional repair followed by deterministic readback, not another evaluator. If the same condition remains, `CONTROL_DEGRADED` stops background retries, preserves the last accepted state, and presents one plain-language checkpoint. A changed anomaly may audit; an unchanged accepted limitation remains silent. Adapter implementation and smoke belong to a focused service task. An old service context gets one observable-start check before one bounded transfer from durable evidence, never repeated wakeups. Preserve both Pending Human Action and the productive Next-Best-Action across intervention: resume or explicitly supersede them using newer decisions. Maintenance dispatch alone cannot end ready management work; release the turn only with productive execution or a concrete wait. Missing native visibility is `LIMITED`, never proof of a stall or permission to duplicate work. See `production-continuation.md` for the existing next-action record and fresh-activity check. Broader Health Review remains for stalled DOD burn, recurring expansion/tax, owner dropout, memory quality, and work hygiene. Project Guard separates durable Return delivery, [checkpoint review](workflows/project-guard.md#checkpoint-review-without-runtime-observation), and observed execution recovery. Where live observation is unavailable, use agreed lease deadlines and expected receipts: one neutral review, manager reconciliation, then quiet or one human checkpoint if unresolved. No inferred idle, automatic evaluator/restart or repeated notice follows from a deadline. Preserve single-delivery ownership, pauses and host permission; ordinary dispatch enrolls actual work at safe boundaries. The existing adapter must prove each supported mode; installing files or proving an installation Return does not establish project-wide coverage.

Check progress toward compass and DOD; blockers, repeated costs, and technical slicing without product progress; unexpected expansion or recurring architecture/data/tooling tax; stale scope, competing candidates, corrections built on rejected work, research or lab outputs missing from the real path, trapped decisions, tracker truthfulness, graph quality and retrieval, and whether the active orchestrator should rotate. After adoption into an existing project or a proven memory miss, route one focused task to build a side-by-side memory Candidate from the accepted brief or earliest reliable baseline. It records each bounded source range in the Source Coverage Ledger, processes it once, lets later evidence explicitly supersede earlier meaning, and deeply inspects only missing, ambiguous or conflicting clusters. Test ordinary future-work queries plus the full source-to-task-to-return-to-memory-update path before human-confirmed integration. Include a `Work Hygiene Check` across every live task, PR, branch, context, worktree, runtime, and monitor that exists: each needs an owner, relation to the current goal/DOD, state, and exit or re-entry trigger, then is classified `ACTIVE`, `WAITING`, `FINISH`, `SALVAGE`, or `RETIRE`. Shared artifacts come from shared metadata; local-only artifacts come from their owner's focused maintenance task, so one machine never claims global cleanliness. Age is a review signal, never deletion authority or a fixed branch limit. The orchestrator only classifies, routes, and stores the check date plus unresolved exceptions in Project State; owning or focused tasks inspect and clean. Preserve evidence and unique work, salvage useful stale work onto the current Accepted Baseline instead of reviving it wholesale, and keep artifact inventory out of the semantic graph except for reusable decisions and lessons.
...
- Project Memory Graph: one current v4 graph with a stable entity spine, atomic requirements/decisions/lessons/commitments, source coverage, Module Contract links, safe pointers, typed routes, and retrieval/application scenarios. Compatible v3, Idea Memory, and Intent Trail are migration inputs, not parallel active truth.
...
At dispatch, material re-brief, blocker, acceptance, and close, update the task contract and the one owning Project State record before announcing the new state; refresh the tracker projection from it without making the projection another control source. Routine progress does not rewrite State, dashboards, Graph, or Alignment. A Team Alignment Delta updates only affected control facts and memory events; Graph advances only after an integrated semantic delta. Legacy `Active Work`, activation, decision, safe-continuation, tracker, and graph summaries are disposable views: regenerate drift model-free and create no new control event. Link history instead of retaining stale or conflicting sections and rotate an Alignment Window when it stops being quickly scannable.
...
4. Create the candidate orchestrator from the current repo/framework in read-only mode. It independently checks Project State v2, DOD Control Line, unresolved leases, Pending Return Inbox, detour gates, high-signal human sources, and graph v4 or its declared v3 migration boundary, then reconstructs executable briefs for current, upcoming, cross-domain, and prior-miss Touch Sets. Matching ids or packet-to-dashboard consistency alone is not memory coverage.
