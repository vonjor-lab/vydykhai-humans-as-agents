...
For every cold-path task, traverse the entity spine in both directions: from the goal through the touched module to its inputs, outputs and dependent consumers, then back to applicable commitments and accepted evidence. Read each touched Module Contract before inspecting current code and tests. A missing, stale or contradictory route, contract, artifact or implementation boundary creates a scoped `MEMORY_COVERAGE_GAP` and bounded `DISCOVERY`; a complete current route proceeds directly to execution.
