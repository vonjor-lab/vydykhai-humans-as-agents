...
- Project Memory Graph v4 links a stable goal-to-module entity map with atomic requirements, decisions, lessons, commitments, ideas, safe operational pointers, source coverage, and current Module Contracts. Compatible v3 remains readable until a separately proven cutover. For explicit `retained-progress-v1` adoption, the orchestrator [prepares reviewed sources](workflows/context-preparation.md); the worker reads and acknowledges actual context, then verifies both retained and new behavior through [Context Route](workflows/context-route.md#bounded-reference-caller). Installing commands does not activate older tasks. The common path needs no hooks; optional native gating covers one trusted declared command. Legacy artifacts remain readable; complete history, fresh-worker interpretation, desktop inheritance and product reliability are not established by bounded CLI proof. Guard gains no semantic timer work.
