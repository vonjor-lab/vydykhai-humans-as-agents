...
Version: 1.28.0 | Status: canonical operating core
...
3. `$project-launch` shapes an unclear goal when needed, then creates the Operating Brief, atomic Project State v2, Project Memory Graph v3, first DOD Control Line, Shared Sync Contract, tracker route, and safe operational pointers. Project State changes become current only after a complete Candidate passes validation, one guarded publication, and exact hash/readback; mismatch restores the last accepted body.
...
Every increment follows a [Context Route](workflows/context-route.md): goal/brief -> scenario and architecture -> affected entities and consumers -> prior decisions and accepted artifacts -> verification. Completeness comes before context length; preserve material rationale and source qualifications. Structural graph validity is not semantic coverage or proof of live behavior. Existing projects inventory and repair their map side by side before claiming the new coverage, without rebuilding all history or interrupting unrelated work. New ideas must not delay the nearest DOD, useful ideas must not disappear, and meaningful intent, decisions, lessons, and operational knowledge must survive people, tasks, and orchestrator changes. Project State is the atomic control snapshot; each current fact has one owning section and its DOD Control Line names accepted proof, exact remaining gap, and next-best-action. Project Memory Graph is reusable semantic memory and advances only for a memory delta, not to mirror ordinary control events. Raw history is linked evidence; each task receives only a working capsule.
...
- Scope freshness, Accepted Baseline, accepted mechanism, complete applicable Memory Brief items, and unchanged compiler output plus application receipt only for declared atomic obligations;
