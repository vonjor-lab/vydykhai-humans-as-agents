...
- The orchestrator decides what, why, when, and who, and maintains the DOD Control Line, sequence, alignment, dispatch, human requests, shared memory, parent closure, and next-best-action. It releases the control turn when ready management work has an observable productive handoff or a concrete wait, relying on durable returns instead of polling. Service input does not replace that obligation. A project-owned external Project Guard survives the orchestrator context: events and a schedule run deterministic checks, while a fresh maximum-profile Governor evaluates only anomalies and routes bounded repair or rotation. Neither performs project work. The maximum-profile orchestrator may use bounded internal advisory agents to improve a control decision, but their analysis is disposable and cannot own a project result, create accepted project evidence, or replace a focused context.
