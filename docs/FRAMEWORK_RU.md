...
Версия: 1.25.0 | Статус: каноническое операционное ядро
...
- Оркестратор решает, что, зачем, когда и кем делается, и поддерживает DOD Control Line, sequence, alignment, dispatch, запросы человеку, общую память, parent closure и next-best-action. Он освобождает control turn, когда готовая управленческая работа передана в наблюдаемое продуктивное исполнение либо имеет конкретное ожидание, полагаясь на durable returns вместо polling. Служебное сообщение эту обязанность не заменяет. Внешний Project Guard принадлежит проекту и переживает context оркестратора: события и schedule запускают deterministic check, а свежий maximum-profile Governor разбирает только anomaly и направляет ограниченный repair либо rotation. Проектную работу они не выполняют. Оркестратор на maximum-profile может использовать ограниченных внутренних advisory agents для улучшения control decision, но их анализ одноразовый и не может владеть проектным результатом, создавать принятое project evidence или заменять focused context.
...
3. `$project-launch` при необходимости помогает сформировать сырую цель, затем создает Operating Brief, атомарный Project State v2, Project Memory Graph v3, первый DOD Control Line, Shared Sync Contract, tracker route и безопасные operational pointers. Изменение Project State становится текущим только после полной Candidate, validation, одной защищенной публикации и точной проверки hash/readback; mismatch восстанавливает последний принятый body.
...
На объявленном return trigger, включая readiness result или human checkpoint, task сначала пишет marked Return Sync с уникальным receipt id и `NO_MEMORY_DELTA`, task-local evidence либо reusable candidates в durable task/tracker outbox, затем пытается отправить тот же id как native wakeup. Action Receipt может доказать действие, но никогда не заменяет terminal Return Sync. Native final, thread read и delivery status не являются источником истины и могут быть пустыми даже при наличии результата в журнале task. На каждом cold path и Governor Check оркестратор совмещает `WRITTEN -> SENT -> RECEIVED -> CONSUMED -> ROUTED` и добавляет парный marked Return Route receipt. Project Guard напрямую сопоставляет эти реальные producer/consumer records по событию записи и active timer. Поэтому потерянное сообщение не теряет результат и не требует опроса человеком. До закрытия lease проверяются sender, evidence, DOD route и next action. Передача между людьми по-прежнему требует exact artifact/revision, environment, воспроизводимых безопасных data при необходимости, recipient access и согласованной проверки. Monitor остается только fallback при отсутствии обоих обычных routes.

Guard adapters используют общий parser outbox, обновляют durable sources по изменениям и перед доставкой сверяют конкретный ожидающий результат и текущего получателя. Несвязанное изменение памяти не теряет ожидающую работу; неполная маршрутизация не закрывает ее. Применение adapter доказывается установленными event и schedule routes, а не только версией комплекта; см. `workflows/project-guard.md`. Первое наблюдаемое действие подтверждает старт, а не добавляет новое согласование; фоновая проверка ограничивает только непроверенную возможность, а не независимую безопасную работу.
...
Project Guard - это operation, а не постоянный context. Один project-owned runner вне оркестратора вызывает deterministic `guard-check` при существенных control events и по независимому timer. Он сопоставляет marked Return Sync и Return Route records со свежей фактической активностью. Incident определяется semantic condition set, а не hash снимка: неизменные принятые ограничения проходят тихо и без модели; новые причины требуют audit. Ожидающая durable работа получает один idempotent wakeup; anomaly или неустраненный повтор запускает свежий maximum-profile Governor с результатом `HEALTHY`, ограниченным `REPAIR` либо подтверждаемым `ROTATE`, без выполнения project work. Реализация и smoke adapter принадлежат focused service task. Вмешательство сохраняет и Pending Human Action, и продуктивный Next-Best-Action: возобновить либо явно заменить их с учетом новых решений. Запуск обслуживания сам по себе не завершает готовую управленческую работу; освобождать тред можно после наблюдаемого исполнения либо конкретного ожидания. Недоступная фактическая активность означает `LIMITED`, а не доказанный простой или разрешение на дубликат. Формат существующего next action и проверка активности описаны в `production-continuation.md`. Более широкий Health Review остается для stalled DOD burn, повторного expansion/tax, выпадения owner, качества памяти и work hygiene.
...
Люди являются событийными участниками системы, а не ее скрытыми диспетчерами. Когда нужен человек, оркестратор сообщает, кто действует, какое суждение принадлежит этому человеку, что проверить или решить, точную ссылку/task/prompt, куда попадет результат, что можно продолжать и какой Return Sync возобновит поток. Этот запрос хранится как один Pending Human Action до ответа, объединения или явной замены; Guard, repair, rotation и несвязанный Return Sync не могут его скрыть. Техническую проверку проводят агенты; человеку задаются понятные продуктовые, визуальные, расходные, внешние, smoke или merge-вопросы по его роли.
...
Оркестратор сам выбирает и применяет skill; человеку достаточно естественных команд:
