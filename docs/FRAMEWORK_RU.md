...
Версия: 1.21.1 | Статус: каноническое операционное ядро
...
- Оркестратор решает, что, зачем, когда и кем делается, и поддерживает DOD Control Line, sequence, alignment, dispatch, запросы человеку, общую память, parent closure и next-best-action. Легкий Governor Check независимо проверяет этот контур по durable state и направляет ограниченный repair либо rotation оркестратора; проектную работу он не выполняет.
...
Один активный оркестратор не означает один вечный context. Он именуется `[ORCHESTRATOR] <проект> — Vydykhai <версия>`, а замененный предшественник - `[RETIRED][ORCHESTRATOR] <проект> — Vydykhai <версия>`; `[FRAMEWORK] Vydykhai — maintenance` зарезервирован для универсального сопровождения. Меняющийся статус хранится в durable Project State, закреплен или выведен на первый план только один active orchestrator, а focused work архивируется после terminal Return Sync и artifact disposition. Governor по наблюдаемому health решает: продолжать, ремонтировать или входить в подтверждаемую rotation; сама compaction является только сигналом.
...
5. Активный orchestrator, Governor baseline, маршрут Execution Lease и Return Sync по схеме durable outbox плюс wakeup проверяются readback, после чего один Project Activation Receipt показывает evidence, безопасные ограничения, первый маршрут и next-best-action.
...
Подключить и доказать repo, участников, inputs, operations текущего DOD, source of truth, privacy rules, компас, атомарные Project State/graph, DOD Control Line и Governor baseline. До первого dispatch зарегистрировать active orchestrator и опубликовать Project Activation Receipt.
...
Governor Check запускается при activation, dispatch/material resume, consumption Return Sync, человеческой коррекции или detour, milestone/update, первом активном событии после 24 часов, двух compaction без независимой проверки, двух control failures одного класса, невозможности восстановить DOD, неизвестном внешнем результате или chat archaeology. Сначала идет deterministic `control-check`; свежий maximum-profile evaluator нужен только для anomaly или semantic health. Он независимо читает durable sources и возвращает `HEALTHY`, ограниченный `REPAIR` либо подтверждаемый `ROTATE`, не выполняя проектную работу. Более широкий Health Review остается для нескольких slices, stalled DOD burn, повторного expansion/tax, выпадения owner, качества памяти и work hygiene.
...
- Поддерживаются один DOD Control Line, один unresolved Execution Lease на инкремент, Return Sync сначала в durable outbox и явные detour return gates. Governor независимо проверяет их и направляет repair либо подтверждаемую rotation; людям не нужно опрашивать tasks или помнить отложенное за систему.
