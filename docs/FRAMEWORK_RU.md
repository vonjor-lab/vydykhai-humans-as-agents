...
Для каждой cold-path задачи маршрут проходится в обе стороны: от цели через затронутый module к его входам, результатам и зависимым потребителям, затем к применимым обязательствам и принятому evidence. До чтения текущего кода и tests читается Module Contract каждого затронутого модуля. Отсутствующий, устаревший или противоречивый маршрут, contract, artifact либо implementation boundary создает локальный `MEMORY_COVERAGE_GAP` и ограниченный `DISCOVERY`; полный актуальный маршрут сразу идет в execution.
