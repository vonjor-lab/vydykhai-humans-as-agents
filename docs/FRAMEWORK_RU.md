...
Версия: 1.24.3 | Статус: каноническое операционное ядро
...
- Оркестратор решает, что, зачем, когда и кем делается, и поддерживает DOD Control Line, sequence, alignment, dispatch, запросы человеку, общую память, parent closure и next-best-action. После наблюдаемого dispatch он освобождает control turn и полагается на durable return route вместо polling работающей task. Внешний Project Guard принадлежит проекту и переживает context оркестратора: события и schedule запускают deterministic check, а свежий maximum-profile Governor разбирает только anomaly и направляет ограниченный repair либо rotation. Проектную работу они не выполняют. Оркестратор на maximum-profile может использовать ограниченных внутренних advisory agents для улучшения control decision, но их анализ одноразовый и не может владеть проектным результатом, создавать принятое project evidence или заменять focused context.
...
Project Guard - это operation, а не постоянный context. Один project-owned runner вне оркестратора вызывает deterministic `guard-check` при activation, dispatch/material resume, записи Return Sync, человеческой коррекции/detour, наступлении lease review, milestone/update, context-loss signal и по независимому timer. Его adapter сопоставляет marked Return Sync и Return Route records, не доверяя native output task. Incident определяется semantic condition set, а не меняющимся hash снимка: неизменное принятое ограничение проходит тихо и без модели, а любая дополнительная или измененная причина требует audit. Ожидающая durable работа получает один idempotent wakeup; anomaly или неустраненный повторный incident запускает свежий maximum-profile Governor. Реализация и smoke adapter выполняются focused service task; оркестратор координирует и принимает ее receipt. Governor независимо читает durable state и фактические context metadata и возвращает `HEALTHY`, ограниченный `REPAIR` либо подтверждаемый `ROTATE`, не выполняя project work. Перед вмешательством сохраняется Pending Human Action; после него оркестратор возвращает этот понятный человеку запрос или явно заменяет его. Более широкий Health Review остается для нескольких slices, stalled DOD burn, повторного expansion/tax, выпадения owner, качества памяти и work hygiene.
