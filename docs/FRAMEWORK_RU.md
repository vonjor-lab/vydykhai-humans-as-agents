...
## Профили по роли
Выбирать по доказанной пригодности и полным затратам, не только по цене/версии. Приоритет у `selectionPolicy`; прежний `modelPolicy` сохранен для старых установщиков:
...
Оркестрация и Discovery с нерешенным выбором сохраняют текущий flagship. Без сопоставимых доказательств сохраняется принятый профиль исполнения либо явно указывается flagship fallback. Это environment mappings, не vendor requirements. Actual model/effort, доказательства, дата/источник и fallback хранятся в Project State; проверяются при bootstrap, update, rotation, rejection/deprecation и активном Health Review не реже раза в семь дней. Профиль передается явно и не подменяется незаметно. Возобновляемая задача сохраняет профиль до re-brief. При оценке затрат учитываются подготовка, проверка и исправления; цены токенов не являются коэффициентами расхода подписочного лимита.
