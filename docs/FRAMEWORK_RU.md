...
Версия: 1.28.0 | Статус: каноническое операционное ядро
...
Каждый инкремент проходит [маршрут контекста](workflows/context-route.md): цель/бриф -> сценарий и архитектура -> затронутые сущности и потребители -> прежние решения и принятые артефакты -> проверка. Полнота важнее длины контекста; причины решений и существенные оговорки источников сохраняются. Правильная структура графа не доказывает полноту понимания или работу продукта. Действующий проект проверяет и уточняет карту рядом с сохраненной исходной версией до заявления нового покрытия, без перечитывания всей истории и остановки независимых задач. Новые идеи не должны откладывать ближайший DOD, полезные идеи не должны теряться, а значимые намерения, решения, уроки и операционные знания должны переживать смену людей, задач и оркестраторов. Project State является атомарным control snapshot: каждый текущий факт имеет один owning section, а DOD Control Line называет принятое proof, точный оставшийся gap и next-best-action. Project Memory Graph хранит переиспользуемый смысл и меняется только при memory delta, а не вслед за обычными control events. Raw history остается evidence, а task получает только рабочую капсулу.
...
- Scope freshness, Accepted Baseline, принятый механизм, полный применимый Memory Brief и его связь с целью, решениями и проверками, а для объявленных atomic obligations - неизменный compiler output и application receipt;
