...
- `PREPARATION`: опциональный ограниченный поиск контекста, `Low` на проверенной недорогой модели. Достаточный актуальный контекст переиспользуется; иначе `context-preparation.md` задает пакет с источниками и областью действия правил для нового исполнителя, без наследования поисковой переписки.
