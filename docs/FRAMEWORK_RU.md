...
Версия: 1.22.0 | Статус: каноническое операционное ядро
...
- Оркестратор решает, что, зачем, когда и кем делается, и поддерживает DOD Control Line, sequence, alignment, dispatch, запросы человеку, общую память, parent closure и next-best-action. Внешний Project Guard принадлежит проекту и переживает context оркестратора: события и schedule запускают deterministic check, а свежий maximum-profile Governor разбирает только anomaly и направляет ограниченный repair либо rotation. Проектную работу они не выполняют.
