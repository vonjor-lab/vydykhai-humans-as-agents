...
Agent context - это логическая граница, а не функция конкретного продукта. Он может быть thread, chat, session, run, workspace или tracker-linked agent. Названия - это навигация, а не второй tracker: focused context и каждая понятная человеку ссылка на работу имеют вид `<work-id> [<track>] [<mode>] — <короткий результат>` на рабочем языке проекта с фиксированными тегами. GitHub использует owning Issue, другой tracker - стабильный task key; PR или context id не заменяют идентичность работы. Обычный execution опускает mode; остальные режимы - `[DISCOVERY]`, `[LAB]`, `[MAINT]` и `[REVIEW]`. Голые номера task, PR или context не используются; artifact называется `PR #456 → #123 [DOD1] — <короткий результат>`. Обезличенный пакет наверх не содержит названий проекта или людей, номеров и ссылок задач, очереди, закрытых материалов, текущего состояния проекта или его следующего действия.

Один активный оркестратор не означает один вечный context. Он именуется `[ORCHESTRATOR] <проект> — Vydykhai <версия>`, а замененный предшественник - `[RETIRED][ORCHESTRATOR] <проект> — Vydykhai <версия>`; `[FRAMEWORK] Vydykhai — maintenance` зарезервирован для универсального сопровождения. Меняющийся статус хранится в tracker и native open/archive state, закреплен или выведен на первый план только один active orchestrator, а focused work архивируется только после terminal Return Sync и artifact disposition. Живые и queued contexts нормализуются при launch, rotation или на следующем cold-path касании; закрытая история массово не переименовывается. Оркестратор нужно менять, когда память перестает быть компактной и надежной.
...
Человек может явно выбрать исключение. Нужно записать причину, границы и условие возврата к обычному пути. Не повторять рекомендацию без новых данных или риска. Команда, prompt, plan или попытка отправки сами по себе не являются завершенным переходом: фактическое состояние как `Trigger / Retrieved rule / Expected / Observed / Evidence / PASS|MISMATCH|UNVERIFIED` считывает context, который владеет переходом. Оркестратор владеет receipts запуска task и Return Sync; действующий context - receipt защищенного доступа; owning task - receipts приемки и live action. Затем оркестратор потребляет receipts задачи, не повторяя smoke, приемку, merge или live action. Action Receipt хранится в существующей task, return или acceptance record, а не в новом artifact. Только `PASS` закрывает переход и остается тихим; `MISMATCH` или `UNVERIFIED` останавливает только этот переход, исправляется или проходит reflection и проверяется снова.
...
Raw Project State, Touch Set, transcripts, полные memory views, task map и размышления оркестратора не передаются в task. По умолчанию task продолжает принятый механизм; новый общий механизм или изменение системы должно быть явно записано либо разрешено через consultation. Lab Mode, Peer Compass Review, model profile и подробные contracts добавляются только когда нужны. Перед patch работающей задачи оркестратор читает события новее последнего Return Sync и совмещает их с более свежими указаниями человека. Затем он создает или готовит context и в launch Action Receipt считывает и исправляет канонический title, а также фактические link, role/profile, active start и Return Sync route. Ответ только с планом, запрошенный profile или непроверенная отправка не считаются прогрессом.
...
После приемки и обязательного human checkpoint Candidate становится новым Accepted Baseline. Rejected Candidate остается только evidence. Человек делает manual merge через task context, который публикует terminal Return Sync с `NO_MEMORY_DELTA` либо компактными memory candidates и любым brief miss. Перед merge, deploy, spend или shared-state mutation считываются owning acceptance и свежие точные actor, environment, revision, permitted mutation и stop conditions; acceptance, merge и deploy остаются разными authorities. Оркестратор интегрирует переиспользуемый смысл, превращает miss в retrieval regression scenario и обновляет затронутые tasks, DOD burn, alignment, parent closure, tracker projection и next-best-action без повторной приемки.

### 7. Health Review

Короткий Health Review проводится после milestone, нескольких принятых слайсов, повторяющихся follow-ups, неожиданного разрастания, остановки DOD burn, выпадения owner, повторных context compaction, когда работа начинает зависеть от археологии по чатам, либо при первой cold-path активности проекта, если с прошлой проверки прошло больше семи дней.
...
- Named human checkpoints, неустранимые blockers и terminal task results автоматически возвращаются в оркестратор; уникальный receipt id совпадает на send, receive, consume и routed next action, а люди не должны опрашивать contexts или синхронизировать routine progress.
