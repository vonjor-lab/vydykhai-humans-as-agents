...
Версия: 1.24.1 | Статус: каноническое операционное ядро
...
Project Guard - это operation, а не постоянный context. Один project-owned runner вне оркестратора вызывает deterministic `guard-check` при activation, dispatch/material resume, записи Return Sync, человеческой коррекции/detour, наступлении lease review, milestone/update, context-loss signal и по независимому timer. Его adapter сам читает id новых Return Sync из durable tracker/outbox и сопоставляет их с routed Project State, не доверяя native output task. Здоровая проверка и неизменный уже доставленный incident проходят без модели и сообщений; ожидающая durable работа получает один idempotent wakeup, а anomaly или неустраненный повторный incident запускает свежий maximum-profile Governor. Он независимо читает durable state и фактические context metadata и возвращает `HEALTHY`, ограниченный `REPAIR` либо подтверждаемый `ROTATE`, не выполняя project work. Перед вмешательством сохраняется Pending Human Action; после него оркестратор возвращает этот понятный человеку запрос или явно заменяет его. Более широкий Health Review остается для нескольких slices, stalled DOD burn, повторного expansion/tax, выпадения owner, качества памяти и work hygiene.
