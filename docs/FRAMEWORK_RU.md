...
Версия: 1.32.2 | Статус: каноническое операционное ядро
...
- Project Memory Graph v4 связывает маршрут от цели к модулям с атомарными требованиями, решениями, уроками, обязательствами, идеями, безопасными указателями, покрытием источников и документацией модулей. Совместимый v3 читается до отдельно проверенного переключения. Обычное обновление включает [активацию](workflows/framework-activation.md); для новых подходящих задач оркестратор [готовит проверенные источники](workflows/context-preparation.md); исполнитель читает и подтверждает фактический контекст, затем проверяет новый результат и ранее принятое поведение через [Context Route](workflows/context-route.md#bounded-reference-caller). Установка команд не активирует старые задачи. Общий путь работает без hooks; опциональный native gate охватывает одну доверенную объявленную команду. Старые artifacts читаются; ограниченная CLI-проверка не доказывает полноту истории, интерпретацию новым исполнителем, наследование в desktop или надёжность продукта. Guard не получает семантическую работу по таймеру.
- Оркестратор решает, что, зачем, когда и кем делается, и поддерживает DOD Control Line, sequence, alignment, dispatch, запросы человеку, общую память, parent closure и next-best-action. Он освобождает control turn, когда готовая управленческая работа передана в наблюдаемое продуктивное исполнение либо имеет конкретное ожидание, полагаясь на durable returns вместо polling. Служебное сообщение эту обязанность не заменяет. Внешний Project Guard принадлежит проекту и переживает context оркестратора: события и schedule запускают deterministic check, а свежий maximum-profile Governor разбирает только anomaly и направляет ограниченный repair либо rotation. Проектную работу они не выполняют. Оркестратор на flagship может использовать ограниченных внутренних advisory agents для улучшения control decision, но их анализ одноразовый и не может владеть проектным результатом, создавать принятое project evidence или заменять focused context.
...
3. `$project-launch` при необходимости помогает сформировать сырую цель, затем создает Operating Brief, атомарный Project State v2, Project Memory Graph v4, первый DOD Control Line, Shared Sync Contract, tracker route и безопасные operational pointers. Он связывает уже существующие актуальные Module Contracts, а недостающую документацию или историческую реконструкцию направляет в отдельный focused task, создающий Candidate. Изменение Project State становится текущим только после полной Candidate, validation, одной защищенной публикации и точной проверки hash/readback; mismatch восстанавливает последний принятый body.
...
При первом запуске, повторном подключении с неизвестной готовностью или применимом обновлении оркестратор проверяет доступную карту ответственности, публичных контрактов, реализации/tests и потребителей, а готовность возможности отдельно отмечает в Project State. Для недостающей документации он сам назначает одного общего focused owner до зависимой новой задачи, переиспользует принятую проверку и не останавливает независимую разрешенную работу. Описание существующего допускается при настройке; изменение границы требует согласованного с человеком плана перехода. Номер комплекта и candidate-файлы сами по себе не доказывают готовность. Только доказанный receipt возвращает `PROJECT_READY`; несущественные gaps дают `PROJECT_READY_WITH_LIMITS`, настоящее решение человека - `NEEDS_DECISION`, а доступ, блокирующий первый безопасный маршрут, - `BLOCKED_BY_ACCESS`. Отсутствующий участник блокирует только пересекающуюся работу.

## Профили по роли
Выбирать по доказанной пригодности и полным затратам, не только по цене/версии. Приоритет у `selectionPolicy` и `effortPolicy`; прежние поля модели/профилей сохранены для старых установщиков, не для текущего запуска:
- `ORCHESTRATOR`: текущий flagship на `Low` для compass, memory, routing, постановки задач, consultation, интеграции и next-best-action. Подтвержденная ошибка управления, сохранившаяся после одной прицельной коррекции, временно переводит его на `High`; после проверки восстановления на безопасной границе он возвращается на `Low`. Ограниченный порядок восстановления описан в `context-routing.md`.
- `DISCOVERY`: текущий flagship на `High` для еще не определенного решения: ограниченного research, продуктового или архитектурного выбора, неясного UX/UI или визуального направления и проектирования эксперимента. Дешевая подготовка может собрать подтверждения, но не заменяет это осмысление.
- `EXECUTION`: эффективный bounded reasoning для определенного решения и границы приемки; `Low` на самой экономной модели, доказавшей пригодность для этого класса задач.
- `PREPARATION`: опциональный ограниченный поиск контекста, `Low` на проверенной недорогой модели. Оркестратор ставит и принимает подготовку по `context-routing.md`: достаточный контекст переиспользуется, иначе первый пакет с источниками получает новый исполнитель; дальнейшие пробелы идут через оркестратор за дополнением тому же исполнителю. Без наследования поисковой переписки, самостоятельного проектирования или управления подготовителем со стороны исполнителя.
Оркестрация и Discovery с нерешенным выбором сохраняют текущий flagship. Без сопоставимых доказательств сохраняется принятый профиль исполнения либо явно указывается flagship fallback. Это environment mappings, не vendor requirements. Actual model/effort, доказательства, дата/источник и fallback хранятся в Project State; проверяются при bootstrap, update, rotation, rejection/deprecation и активном Health Review не реже раза в семь дней. Профиль передается явно и не подменяется незаметно. Возобновляемая задача сохраняет профиль до re-brief. Датированные коэффициенты кредитной стоимости служат расчетным ориентиром для подготовки, управления, исполнения, проверки и исправлений; их уточняют по атрибутируемому расходу на практике, не выдавая расчет за измеренную экономию подписочного лимита.
...
План агента не может отменить более позднюю человеческую корректировку. Прямое управление человека сохраняется в task contract/lease до явного возврата координации; новые указания читаются до управляющей команды, противоречия решаются диалогом, а не конкурирующими поручениями. Существенная поправка является memory event: нужно выяснить, отсутствовало ли знание, не нашлось, не применилось или не проверилось, и сохранить исправленный смысл до продолжения зависимой работы. Останавливается только затронутый scope.
...
Каждый инкремент проходит [маршрут контекста](workflows/context-route.md): цель/бриф -> outcome/journey -> module/capability -> сущности, contracts, data, artifacts и systems -> текущий смысл и обязательства -> потребители -> принятые артефакты и проверка. Полнота важнее длины контекста; причины решений и существенные оговорки источников сохраняются. Graph v4 отделяет стабильную карту сущностей от атомарного текущего смысла и связывает каждый устойчивый module/capability с актуальным [Module Contract](workflows/module-contract-template.md). Правильная структура графа не доказывает полноту понимания или работу продукта. Действующий проект один раз проходит ограниченные диапазоны источников от принятого brief до текущего watermark и строит v4 Candidate рядом с совместимым v3, не останавливая независимые задачи. Новые идеи не должны откладывать ближайший DOD, полезные идеи не должны теряться, а значимые намерения, решения, уроки, обязательства, документация и операционные знания должны переживать смену людей, задач и оркестраторов. Project State является атомарным control snapshot; Project Memory Graph хранит переиспользуемый смысл и меняется только при memory delta. Raw history остается evidence, а task получает только рабочую капсулу.
...
- Просьба человека «запомнить и вернуться» является отдельным узлом `COMMITMENT`, а не обычной историей. Он связывается с затронутыми сущностями и хранит актуальный смысл, source, aliases и trigger, применимость/время, owner, условие возврата или закрытия, evidence и нерешенный вопрос человеку. Если момент не определен, вопрос возвращается при первом совпавшем планировании, а не разрешается агентом самостоятельно.
...
- Outcomes, actors, journeys, modules, capabilities, entities, surfaces, contracts, data, artifacts, systems и operations получают стабильные anchors и aliases. Они связываются простым набором `part-of / serves / produces / consumes / depends-on / stored-in / constrains`, который читается в обе стороны. Один узел хранит один независимо меняющийся смысл: `INVARIANT`, `REQUIREMENT`, текущий `DECISION`, урок неудачного пути `LESSON`, открытый `COMMITMENT`, будущую `IDEA` или безопасный операционный `POINTER`. Защищенный `POINTER` исполним только тогда, когда текущая память называет owner, protected reference без самого значения, environment и scope, разрешенный non-destructive route, последнюю safe check с временем, результатом и source, а также expiry или условие повторного входа.
...
При каждом cold-path brief, re-brief, dispatch, решении consultation или sequence, parent acceptance, milestone и rotation оркестратор определяет точные anchors и aliases из Touch Set, до фильтрации dormant items пересекает совпавшие открытые обязательства памяти и checkpoints, добавляет semantic candidates, проходит релевантные relations до восстановления полного маршрута от цели до проверяемого результата и фильтрует по source precedence, status, scope, applicability и supersession. Он передает все применимые смыслы без фиксированного лимита узлов как исполнимые пункты `Because / Apply / Avoid / Verify / Source`; одних id недостаточно. Если нужный смысл не доказан, ставится `MEMORY_COVERAGE_GAP`. Когда пропуск, объединение или перестановка разрушат неделимую последовательность, матрицу сценариев, safety gate или уже найденный compression miss, только эта часть проходит через compiler-derived [Executable Memory Brief](workflows/memory-brief-envelope.md) и требует связанный application receipt; обычный контекст остается текстом. Compaction или migration сначала проверяется тремя-четырьмя дешевыми обычными unhinted вопросами из реальных задач. Любой miss останавливает promotion до широкого запуска агентов; затем выполняется только targeted regression по найденному дефекту, atomic shadow integration и human-confirmed cutover. Прошедшие до этого synthetic cases не доказывают semantic coverage. Неполный protected pointer блокирует только затронутое действие до повторного запроса секрета, исторической реконструкции или live mutation; указатель восстанавливается по связанному durable evidence, retrieval повторяется, а работа продолжается только после доказательства полного безопасного указателя и актуальной non-destructive check. Перед интеграцией перечитывается watermark, evidence сохраняется, а на hot-path continue retrieval не повторяется.

Для каждой cold-path задачи маршрут проходится в обе стороны: от цели через затронутый module к его входам, результатам и зависимым потребителям, затем к применимым обязательствам и принятому evidence. Для каждого модуля определяется consume/change/create по Module Contract. Подключение использует публичную документацию и фиксированную поставку без чтения внутреннего кода; согласованное развитие добавляет maintainer design и собственный код/tests. Новые/изменяемые границы согласуются с человеком, принятые версии сохраняются; готовность упаковки доказывается независимым подключением. Композиция использует публичные границы, не разворачивает все внутренние зависимости. Отсутствующий, устаревший или противоречивый маршрут, contract, artifact либо implementation boundary создает локальный `MEMORY_COVERAGE_GAP` и ограниченный `DISCOVERY`; полный актуальный маршрут сразу идет в execution.
...
- Research Context: восстановление недоказанного контекста или ограниченный вопрос решается в `DISCOVERY` без изменений product code. После incorporation контекст закрывается, кроме явно назначенного ведущего по согласованности нескольких задач; выбор, ожидания, консультации, объяснение человеку и выход описаны в [Context Route](workflows/context-route.md). Ведущий не становится вторым диспетчером.
- Lab Mode: изолированная реализация или эксперимент сокращают риск, стоимость или время обратной связи. До запуска определить решение, Accepted Baseline, одну главную переменную, согласованные entry/mechanism и условия теста, проверяемый человеком proof, stop/burn limit и путь promote/reject/re-brief. Отступления согласовываются с владельцем решения: отключенный AI или fixture не доказывают работу пропущенного producer и не оправдывают его замену. Закрывается согласованный вопрос Lab; продуктовая приемка дополнительно требует разрешенного production transfer, tests и risk-based real-flow smoke. Успех Lab или исчерпание лимита сами по себе не разрешают перенос.
...
Research уменьшает неопределенность. Lab уменьшает стоимость исполнения. Task доставляет принятый продуктовый или enabling результат. [Context Routing](workflows/context-routing.md#fit-with-existing-work) разделяет поиск фактов Preparation и выбор решения Discovery во всех режимах: исследование может начинаться с явных пробелов; полный контекст реализации является возможным результатом, а не рекурсивным условием запуска. Достаточные пакеты переиспользуются; обязательная документация входит в принимаемый Candidate.
Discovery Decision Packet называет выбранный подход, отвергнутые варианты и уроки, затронутые entities/interfaces, evidence, риски и нерешенные owner decisions. Оркестратор интегрирует его до исполнения. После этого checkpoint ведущий сохраняет только названные обязанности по решению и review, использует тот же Return Sync и конкретные ожидания; режим требует доказанного охвата всех активных ответственных Guard, иначе применяется обычный Discovery. Новая постоянная роль, личная память или расписание не добавляются.
...
- Scope freshness, Accepted Baseline, принятый механизм, полный применимый Memory Brief, актуальные Module Contracts и implementation boundaries, ожидаемый documentation impact, а для объявленных atomic obligations - неизменный compiler output и application receipt;
...
### 4. Исполнение
Task context сначала проверяет фактический cwd, чтение источников, разрешенную запись отчета и доставку по `workflows/task-context-handoff-template.md#execution-readiness`, затем начинает безопасное наблюдаемое действие без повторения согласованного планирования. Пустой API или отсутствие отчета требуют сверки исхода конкретного хода, а не слепого повтора. Task отвечает за implementation, debugging, tests, исправления и durable checkpoints до named human checkpoint, неустранимого blocker или terminal result. Он не запускает project launch, shaping, alignment, orchestration или Governor workflows.
...
Нельзя механически повышать reasoning застрявшей задачи по лестнице effort. Сначала различить implementation defect, слабый acceptance oracle и недостающую проработку решения. Затем построить новую попытку от Accepted Baseline с учетом evidence, сделать re-brief либо запустить bounded Discovery. High-consequence execution может оставаться на эффективном profile только при явных invariants, deterministic guards и review evidence и project coherence оркестратором до human merge или action approval; task acceptance остается в task.

На объявленном return trigger, включая readiness result или human checkpoint, task использует установленный canonical writer и сначала пишет marked Return Sync с уникальным receipt id и `NO_MEMORY_DELTA`, task-local evidence либо reusable candidates в durable task/tracker outbox, затем пытается отправить тот же id как native wakeup. Уточнение статуса хранится отдельной detail, а не в придуманной slash-форме. Общий reader принимает только известные legacy aliases/qualifiers и отвергает смешанные aliases, поэтому старую корректную историю не нужно переписывать. Action Receipt может доказать действие, но никогда не заменяет terminal Return Sync. Native final, thread read и delivery status не являются источником истины и могут быть пустыми даже при наличии результата в журнале task. На каждом cold path и Governor Check оркестратор совмещает `WRITTEN -> SENT -> RECEIVED -> CONSUMED -> ROUTED` и добавляет парный marked Return Route receipt. Project Guard напрямую сопоставляет эти реальные producer/consumer records по событию записи и active timer. Поэтому потерянное сообщение не теряет результат и не требует опроса человеком. До закрытия lease проверяются sender, evidence, DOD route и next action. Передача между людьми по-прежнему требует exact artifact/revision, environment, воспроизводимых безопасных data при необходимости, recipient access и согласованной проверки. Monitor остается только fallback при отсутствии обоих обычных routes.

Guard adapters используют общий parser outbox, обновляют источники по изменениям и перед доставкой сверяют ожидающий результат и текущего получателя. Несвязанное изменение памяти не теряет ожидающую работу; неполная маршрутизация не закрывает ее. Применение доказывается установленными event и schedule routes, а не только версией комплекта (`workflows/project-guard.md`). Применяется `workflows/task-context-handoff-template.md#return-authorization`: проверить актуальное поручение человека, получателя и разрешенный состав отчета; сохранять результат как `WRITTEN` до успешной отправки. Guard или другой журнал не обходят запрещенную передачу под видом потерянного уведомления. Действующее согласие сохраняется, если границы или политика среды не требуют нового; receipt фреймворка не предоставляет прав среды. Первое действие подтверждает старт, а не добавляет согласование. Фоновые проверки ограничивают только затронутую возможность, не независимую безопасную работу.
...
Task возвращает `Documentation impact: NONE` либо обновляет затронутые Module Contracts в той же Candidate и называет точные файлы/разделы и кандидаты маршрутов графа. Оркестратор проверяет согласованность принятого поведения и документации, затем до запуска зависимой работы интегрирует только переиспользуемый смысл и маршруты общей памяти. Сам он не редактирует продуктовый код или документацию модулей, а task не переписывает общую память напрямую.

### 7. Health Review

Project Guard - это operation, а не постоянный context. Один project-owned runner вне оркестратора вызывает deterministic `guard-check` при существенных control events и по независимому timer. В режиме наблюдаемого восстановления он сопоставляет marked Return Sync и Return Route records со свежей фактической активностью. Runner хранит единственный incident ledger вне Project State; incident определяется semantic condition set, а не hash снимка. Связанные изменения собираются в одну проверку, обычные State revisions и устаревшие display views не вызывают модель, а Project Memory Graph не затрагивается без изменения смысла памяти. Обертки команд сохраняют реальный exit status. Живой lock остается занятым; только доказанно stale same-host lock изолируется и проверяется заново после сверки внешнего исхода, без права на replay. Ожидающая durable работа получает один idempotent wakeup; только настоящая anomaly или доказанно неотработанный wake запускает свежий maximum-profile Governor. `HEALTHY` закрывает incident; `REPAIR` допускает одну транзакционную правку и deterministic readback без второго evaluator. Если та же причина остается, `CONTROL_DEGRADED` прекращает фоновые повторы, сохраняет последний принятый State и показывает человеку один понятный checkpoint. Новая anomaly может пройти audit, а неизменное принятое ограничение остается тихим. Реализация и smoke adapter принадлежат focused service task. Старый service context получает одну проверку observable start, после чего возможен один bounded transfer от durable evidence, но не повторные wakeups. Вмешательство сохраняет и Pending Human Action, и продуктивный Next-Best-Action: возобновить либо явно заменить их с учетом новых решений. Запуск обслуживания сам по себе не завершает готовую управленческую работу; освобождать тред можно после наблюдаемого исполнения либо конкретного ожидания. Недоступная фактическая активность означает `LIMITED`, а не доказанный простой или разрешение на дубликат. Формат существующего next action и проверка активности описаны в `production-continuation.md`. Более широкий Health Review остается для stalled DOD burn, повторного expansion/tax, выпадения owner, качества памяти и work hygiene. Project Guard различает доставку Return, [проверку контрольных точек](workflows/project-guard.md#checkpoint-review-without-runtime-observation) и восстановление по наблюдаемому состоянию выполнения. Если фактическая активность недоступна, используются согласованные сроки и ожидаемые результаты в существующих leases: одно нейтральное обращение, сверка оркестратором, затем тишина либо один запрос человеку при отсутствии решения. Просрочка не доказывает простой и не запускает evaluator, перезапуск или повторные уведомления. Сохраняются единственный владелец доставки, паузы и разрешения среды; реальные задачи подключаются при обычном запуске или безопасном возобновлении. Адаптер доказывает каждый режим отдельно: установка файлов и передача отчёта об установке не означают защиту всего проекта.

Проверяются продвижение к compass и DOD; blockers, повторные затраты и technical slicing без продуктового прогресса; неожиданное разрастание или повторяющийся architecture/data/tooling tax; stale scope, конкурирующие Candidates, исправления на rejected work, research или lab outputs вне real path, потерянные решения, правдивость tracker, качество и retrieval графа, необходимость сменить active orchestrator. После подключения к действующему проекту или доказанного memory miss оркестратор направляет одну focused task собрать side-by-side Candidate от принятого brief либо самого раннего надежного baseline. Она отмечает каждый ограниченный диапазон источника в Source Coverage Ledger и обрабатывает его один раз; поздний evidence явно заменяет ранний смысл, а глубоко разбираются только отсутствующие, неоднозначные или конфликтующие кластеры. До подтвержденной человеком интеграции проверяются обычные вопросы о будущей работе и полный путь source -> task brief -> применение -> acceptance/return -> memory update. Внутри проводится `Work Hygiene Check` всех существующих live task, PR, branch, context, worktree, runtime и monitor: у каждого должны быть owner, связь с текущими goal/DOD, состояние и условие выхода либо возврата, после чего он получает класс `ACTIVE`, `WAITING`, `FINISH`, `SALVAGE` или `RETIRE`. Shared artifacts читаются из общей metadata, local-only artifacts проверяет их owner в отдельной focused maintenance task, поэтому одна машина не может объявить чистой всю команду. Возраст запускает review, но не удаление и не фиксированный лимит веток. Оркестратор только классифицирует, направляет и хранит в Project State дату проверки с нерешенными исключениями; owning или focused tasks проверяют и очищают. Evidence и уникальная работа сохраняются, полезная stale-работа переносится на текущий Accepted Baseline вместо полного оживления старого context, а inventory artifacts не попадает в semantic graph, кроме переиспользуемых решений и уроков.
...
- Project Memory Graph: один текущий v4 graph со стабильным сущностным каркасом, атомарными requirements/decisions/lessons/commitments, покрытием источников, ссылками на Module Contracts, безопасными pointers, типизированными routes и сценариями retrieval/application. Совместимый v3, Idea Memory и Intent Trail являются migration inputs, а не параллельной active truth.
...
При dispatch, существенном re-brief, blocker, acceptance и close task contract и единственный owning record Project State обновляются до объявления нового состояния; tracker projection строится из него и не становится вторым control source. Routine progress не переписывает State, dashboards, Graph или Alignment. Team Alignment Delta меняет только затронутые control facts и memory events; Graph продвигается лишь после интегрированного semantic delta. Legacy `Active Work`, activation, decision, safe-continuation, tracker и graph summaries являются disposable views: их drift исправляется model-free без нового control event. История остается по ссылкам, а Alignment Window ротируется, когда перестает быстро читаться.
...
4. Candidate orchestrator создается из актуальных repo/framework в read-only режиме. Он независимо проверяет Project State v2, DOD Control Line, unresolved leases, Pending Return Inbox, detour gates, high-signal human sources и graph v4 либо объявленную границу миграции v3, затем восстанавливает исполнимые briefs для current, next, cross-domain и prior-miss Touch Sets. Совпадение ids или согласованность Packet с dashboard сами по себе не являются memory coverage.
...
- Implementation не начинается без goal, boundary, DOD impact, human checkpoint и verification route. Эта цель сохраняется после уточнений, побочных вопросов и ошибок внутренних тестов: исполнитель продолжает исправления до согласованного результата/контрольной точки, явной паузы или подтвержденного неразрешенного препятствия. Обновление охватывает всех незавершенных исполнителей на безопасном этапе или перед возобновлением; проверка их файлов и прочтения инструкций не заменяется версией менеджера. Продуктовый код и прямое управление человека сохраняются (`framework-activation.md`, `task-context-handoff-template.md`).
...
- Маршрутизация учитывает пригодность и затраты: flagship для оркестрации и нерешенного Discovery, проверенные экономные Execution и опциональная Preparation. Actual mappings, доказательства и дата хранятся в Project State; vendor id не фиксируется, профиль не подменяется незаметно.
