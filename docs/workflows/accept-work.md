...
For a zero-spend or no-mutation contract, disable the dangerous capability when practical and record relevant counters before and after. That restriction limits what the test proves: a prepared fixture or disabled AI cannot establish fresh recognition or another unexercised producer. Missing access is a blocked proof, not a reason to replace the accepted mechanism. Agree any departure from the promised test conditions with the decision owner; retain the original path as pending until actually verified. Any accidental call, spend, or mutation remains evidence and cannot later be described as zero.

For a reusable module or end-to-end capability, compare the claimed applicability with evidence, not just passing examples. Before implementation, the owner defines supported input families, meaningful variations, failure behavior and the promised entry/output; acceptance checks independent held-out/edge cases and clean-start repetitions on one fixed Candidate. Replaying one input, renaming ids, iterating intermediate results or changing code between cases cannot establish independent successes. Disclose all normalizations, manual steps, seeded intermediates, attempted failures and untested stages. A documented production preprocessing step is valid only if inside the claimed reproducible route. Finite tests bound confidence; they never prove "all inputs".
If that evidence is missing, keep reusable readiness pending even when the example is useful. `ACCEPT_WITH_FOLLOWUPS` cannot defer the promised reuse proof as optional work: a narrower accepted outcome requires the decision owner's explicit scope change. Return Sync, Module Contract and shared memory must carry the same applicability and unresolved obligations, so later integration cannot promote sample success into a ready donor. Existing frozen new-behavior expectations express the required proof; a green reference-runner receipt alone cannot certify its semantic adequacy.
Derive the coverage inventory from the authoritative input/contract, not only the subset the algorithm processed successfully. Every required entity, region or stage must be accounted for as satisfied, failed or explicitly unsupported; skipped/unexamined members cannot disappear into a green result. Review that the oracle checks this completeness independently of the implementation's selection logic.
...
- write that complete marked Return Sync to the durable task/tracker outbox first, then use its single accepted notification owner under Return Authorization (native by default, never parallel Guard delivery). An Action Receipt never substitutes for Return Sync, including a readiness or human-checkpoint result. Report only through `SENT`; the orchestrator owns `RECEIVED -> CONSUMED -> ROUTED`, the paired marked Return Route receipt, and lease closure. Native final text and task/thread reads are non-authoritative and may be empty; native loss does not justify human polling or recreating an already durable result.
