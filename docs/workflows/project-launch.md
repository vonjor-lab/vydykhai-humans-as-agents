...
9. Reuse the registered active orchestrator when healthy and create one only when absent. Replace it only through confirmed rotation, never by leaving two active contexts. Name it `[ORCHESTRATOR] <project> — Vydykhai <version>`, read back its actual title and effective profile, and pin or foreground exactly that one context when supported; Project State owns the current pointer.
