...
7. Resolve capability-and-cost profiles under the core: flagship maximum `ORCHESTRATOR` and deep bounded `DISCOVERY`, proven efficient `EXECUTION` and opt-in retrieval `PREPARATION`; record actual mappings, capability evidence, source/date and fallback. No silent downgrade.
...
10. Install one project-owned Project Guard outside the active orchestrator context using the cheapest available independent scheduler. Select supported capabilities through `project-guard.md#checkpoint-review-without-runtime-observation`; missing runtime metadata need not block neutral checkpoint review. For observation/recovery prove event and timed routes, deterministic `guard-check`, actual-context and `Human attention` read, one runner-owned incident ledger, native wakeup, maximum evaluator only for anomaly, and the one-repair circuit breaker. Routine State revisions and view drift stay model-free; Graph is not rewritten without a memory delta. Use one real emitted Return Sync/Route pair, then prove the installed schedule produces `NOOP` with no wake, queued message, or model call; malformed or mismatched routing must audit. Adapter implementation and smoke stay in a focused service task. The runner targets the Project State pointer so it survives rotation; if no independent scheduler exists, report the exact `LIMITED` boundary instead of claiming background recovery.
...
Create or update one compact Project State using `project-state-template.md`. Store each current fact once in its owning control section. Keep the activation receipt immutable and treat tracker plus legacy Active Work/decision/safe-continuation summaries as views. Render one complete Candidate away from the accepted body, run `control-check` and retain its SHA-256, publish once with an exact-current guard when available, then export and verify the exact hash plus validation. Restore and verify the last accepted body on mismatch; never let a partial or failed write become current or append state after the end marker.

Create one Project Memory Graph when the first reusable requirement, decision, lesson, commitment, idea, or safe operational pointer must survive its source context. Build its v4 entity spine from outcomes/journeys through modules/capabilities to entities, contracts, data, artifacts, systems, and operations. Link every durable module/capability to one current project-owned Module Contract and implementation boundary. If one is missing, route its creation to a focused owning task Candidate; the orchestrator never writes product documentation. Keep one meaning per node and link operations to protected runbooks or secret systems.

For an existing project with meaningful history, route one focused side-by-side reconstruction without blocking safe current work or mutating accepted shared state. Start from the accepted brief or earliest reliable baseline and build a Source Coverage Ledger through the current watermark across accessible human dialogue, meetings, tasks, participant packets, PRs, code, and accepted artifacts. Process each bounded source range once; reread only changed revisions and declared gaps. In that focused Candidate, build the entity spine and create or repair Module Contracts first, then attach atomic current meaning. Let later evidence explicitly supersede earlier meaning and retain unavailable or conflicting coverage rather than guessing. First use cheap fresh actors on ordinary unhinted real-task questions for current, next, cross-domain, prior-miss, and sufficient-context cases. Carry representative routes through a task brief, application result, Return Sync, and memory-update replay. Any miss stops broad evaluation; correct only the exposed route, run targeted regression, then use atomic shadow integration and confirmed cutover. Do not copy full transcripts or model narration into current memory.

For an old graph, follow the participant consolidation in `context-route.md`: each relevant orchestrator contributes its own source-backed delta or coverage gap, one integration owner builds the side-by-side v4 Candidate, and each available participant retrieves the accepted shared meaning after cutover. Preserve ids/sources, compare `CURRENT / NEXT / CROSS_DOMAIN / PRIOR_MISS`, show loss/conflict/delta, and switch only after structural validation, source-to-task application proof and human confirmation. Map old Idea Memory and Intent Trail before making them read-only evidence. Run `node scripts/vydykhai.mjs control-check --state <exported-state.md> --graph <exported-graph.md>` before activation or schema cutover. Missing participants limit affected coverage, not unrelated work.
