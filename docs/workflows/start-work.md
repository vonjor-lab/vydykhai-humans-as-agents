...
Map every outcome to existing module capabilities first and declare `consume / change / create` per affected module under [Module Contracts](module-contract-template.md#use-change-or-create). Tell the person the public boundary, preserved modules and proposed changes; reuse agreed boundaries, but obtain explicit agreement for new/changed boundaries before mutation. Integration consumes a pinned release, not an implicit license to develop it. Preserve existing accepted behavior and its limits; packaging remains unproven until the connection check passes, not grounds to demote or rewrite all existing code. Do not halt independent work to modularize the whole project.
...
Use [Context Routing](context-routing.md#fit-with-existing-work) across these modes: missing known facts may use PREPARATION; choosing a solution requires deep Discovery. Scoped research can start with explicit gaps and a permitted report route; a complete implementation package is its possible result, not its entry requirement. Reuse a sufficient Discovery return instead of another compulsory preparation pass.
...
For each consumed module, read its public contract, applicable obligations and fixed release/dependency evidence; inspect only consumer wiring as needed, not producer source or maintainer rationale. A changed/new module uses graph -> Module Contract -> maintainer design -> current owned code/tests. These are preparation paths, not compulsory Discovery rituals. A scoped mismatch uses public diagnostics/cheap factual retrieval first, then bounded Discovery for unresolved design; internal edits need explicit change authority.
...
For new implementation tasks touching durable modules with prior obligations or acceptance behavior, use `retained-progress-v1` and [Context Preparation](context-preparation.md) by default as part of [Framework Activation](framework-activation.md). The orchestrator reviews sources and plan; the worker reads and acknowledges actual context before `context-run`. For reusable capabilities, pin supported input families, meaningful variations, failure behavior and the promised entry/output in the existing brief/Module Contract; freeze independent reuse proof as new-behavior expectations under `accept-work.md`, not merely a passing sample. Missing critical coverage remains scoped pending/LIMITED for that implementation, not independent authorized research. Old workers enter only at safe rebrief/readback; the common path needs no hooks.
...
Module contracts / intent: <consume/change/create per module; agreed boundary; public contract and release identity for use, maintainer docs and owned implementation/tests for change>
...
Execution Lease / durable return: <PREPARED identity, review-by, exact recipient, approved outbox/wakeup, current human source and permitted report data under task-context-handoff-template.md#return-authorization; if checkpoint mode is selected, agreed structured checkpoint + expected receipt id + sole delivery owner under project-guard.md#checkpoint-review-without-runtime-observation>
...
Show the brief, task map, sequence, ownership, checkpoints, Memory Brief, open decisions, and risks. Ask for human approval when this introduces a new goal, boundary, shared-tracker commitment, or material change. A direct human “do it” for the reviewed plan is approval; an ordinary continuation within that approved contract needs no second approval.

Require each task to read its supplied graph route and mode-appropriate Module Contracts, verify public connections for consume or owned code for change/create, update affected contracts/map links in the same Candidate, and return exact documentation impact with acceptance evidence. Do not reread producer internals to accept unchanged consumption.

After the required approval is established, create/update each task's one current contract, formal parent/dependencies where supported, and tracker projection together, then return the sequence to the Framework Orchestrator. Compile any declared atomic Memory Brief before dispatch; a hand-written manifest or shortened render is invalid. Every implementation task starts in role `EXECUTION` on the efficient profile: it continues the accepted mechanism, performs the first safe observable action in the launch/resume turn, resolves ordinary failures autonomously, consults only at a material undeclared boundary, never climbs effort levels mechanically, and runs `$accept-work`. At every declared readiness, checkpoint, blocker, or terminal return trigger it writes the full marked unique-id Return Sync to the durable outbox before notification by its single accepted owner under Return Authorization (native by default, never parallel Guard delivery); an Action Receipt never substitutes, and the orchestrator later appends the paired marked Return Route receipt. Its title is `<work-id> [<track>] [<mode>] — <short outcome>` using the owning Issue or stable tracker key, never a PR; normal execution omits the mode. Before declaring launch, the orchestrator reads back title, actual link, role/profile, exact base, `PREPARED -> STARTED` lease evidence, and return route. The task does not run orchestration or alignment workflows.
