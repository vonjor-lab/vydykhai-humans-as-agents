...
For a newly declared `retained-progress-v1` task, the orchestrator uses [Context Preparation](context-preparation.md) to select original source exports, review and confirm the plan. The assigned worker then reads and acknowledges the actual context before `context-run` work under [Context Route](context-route.md#bounded-reference-caller). Missing source coverage remains pending. The common path needs no hooks; installing it does not activate older tasks. Prove the project's supported route and state LIMITED for unproven host coverage.
