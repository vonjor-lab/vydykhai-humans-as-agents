...
For new tasks touching durable modules with prior obligations or acceptance behavior, use `retained-progress-v1` and [Context Preparation](context-preparation.md) by default as part of [Framework Activation](framework-activation.md). The orchestrator reviews sources and plan; the worker reads and acknowledges actual context before `context-run`. Missing critical coverage remains scoped pending/LIMITED. Old workers enter only at safe rebrief/readback; the common path needs no hooks.
