...
- Merge semantic duplicates into one graph node and map every old id to `covered`, `superseded`, `evidence only`, or `ambiguous` in the graph's Legacy Source Map. Preserve source links and rejected-path lessons without copying chronology. For any promise to remember or revisit later, also preserve current meaning, capability aliases and trigger, applicability/timing with plan/checkpoint relation, and the pending human question.
...
- Finish only after representative Touch Sets and ordinary future-work queries retrieve the expected current decisions, open recall commitments, sources, and human gates, and every old id has a disposition. Id coverage alone is insufficient. Then mark the Intent Trail legacy/read-only; all future retrieval and updates use the Project Memory Graph.
