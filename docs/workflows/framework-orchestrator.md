...
Governor state / receipt / independent check / rotation signals:
Can continue:
Next-best-action:
```
...
At project activation, dispatch or material resume, Return Sync consumption, human correction or detour, milestone/update, and the first active event after 24 hours, run a **Governor Check**. Also run it after two context compactions without an independent check, two same-class control failures, ambiguous side-effect outcome, DOD reconstruction failure, or chat archaeology. First run deterministic structural and reconciliation checks; use a fresh maximum-profile evaluator only for a mismatch or semantic health question. It reads durable sources independently rather than accepting orchestrator self-report, implements no project work, and returns `HEALTHY`, `REPAIR`, or `ROTATE`. Its receipt must name the exact current Project State event; a stale healthy receipt cannot close a newer triggered transition. `REPAIR` pauses only the invalid transition and names one bounded repair; `ROTATE` freezes new dispatch and enters the confirmed rotation path. Compaction alone is a signal, not an automatic replacement.
...
Run Health Review after milestones, several slices, repeated same-class corrections, stalled DOD burn, unexpected expansion, recurring architecture/data/tooling tax, owner dropout, stale scope/dashboards, or chat archaeology. Governor applies the shorter control thresholds: first active event after 24 hours, two compactions without independent check, or two same-class failures.
