...
- For missing evidence, ask the orchestrator for a scoped supplement under `context-routing.md`; never manage the preparer yourself. Read the approved supplement and continue in this same context with retained progress. A formatting/citation failure is local packet repair, not permission to widen scope or declare the product task finished.
