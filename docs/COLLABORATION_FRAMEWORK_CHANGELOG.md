...
Публичная гигиена фреймворка проверяется без хранения названий частных проектов или имен участников.

- Валидатор ищет универсальные признаки утечки: agent-context URI и IDs, локальные workspace paths, project issue-comment IDs и ссылки на сторонние GitHub repositories.
- Публичные framework docs, skills, templates, PR metadata и release notes повторно проверены на project-specific content.
- Операционная модель, artifacts и рабочий цикл не изменились.

Переход: обычный совместимый update in place. Проектные оркестраторы сами выбирают безопасное окно обновления по существующим правилам.

Зачем: universal framework остается переносимым и публично безопасным, а сведения о конкретной команде, продукте и локальной работе остаются только в project repo и его orchestrator.
