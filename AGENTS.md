...
- Route models by capability and cost: retain flagship maximum reasoning for orchestration and deep bounded reasoning for solution discovery; use the lowest proven capable model for bounded preparation and Low-ready execution. Preparation is opt-in; absent comparable evidence, retain the accepted profile or explicit flagship fallback. Record actual model/effort, evidence and fallback; never pin a universal vendor id. Keep `defaultAgentProfile` only for older updater compatibility; `agentRoutingPolicy` is authoritative.
