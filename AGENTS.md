...
- Keep `doctor` limited to installed-kit integrity. Live repo/tracker permissions, participant self-readiness, coordination inputs, first-DOD operations, compass, and control-loop proof belong to the evidence-backed `$project-launch` receipt; never let one machine certify another.
