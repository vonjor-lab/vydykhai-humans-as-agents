...
- Route models by capability and cost: use flagship/Low for orchestration with bounded temporary High recovery and flagship/High for solution discovery; use the lowest proven capable model for bounded preparation and Low-ready execution. Preparation is opt-in; absent comparable evidence, retain the accepted profile or explicit flagship fallback. Record actual model/effort, evidence and fallback; never pin a universal vendor id. Keep `defaultAgentProfile` only for older updater compatibility; `agentRoutingPolicy.selectionPolicy` and `effortPolicy` are authoritative; its legacy profile fields remain only for older installer compatibility.
...
- Preserve source precedence, explicit human checkpoints, asynchronous participant visibility, dashboard freshness, Work Hygiene Check, monitor cleanup, and orchestrator rotation. Shape work around documented module boundaries: consume fixed public releases without producer-source exploration; change/create modules only within explicit human-approved scope. Packaging needs independent consumer proof, not only a successful Lab example.
