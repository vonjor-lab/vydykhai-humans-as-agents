...
7. Establish one coordination-input route. Prefer direct least-privilege source access for each relevant orchestrator. When that is unavailable or unnecessary, name one intake owner whose accessible source is distilled into an approved, traceable shared delta. Treat raw meetings, transcripts, chats, and notes as inputs, not project truth.
   Installing the kit supplies `context-prepare` and `context-run`; it does not activate older tasks. For explicit `retained-progress-v1` adoption, the project orchestrator supplies relevant source exports, reviews the plan under `docs/workflows/context-preparation.md`, and checks its own supported route. The worker reads and acknowledges actual context before execution/verification. This common route needs no hooks. A separately trusted native adapter covers one declared command only; complete live history and desktop inheritance require their own evidence. Keep unsupported coverage LIMITED.
