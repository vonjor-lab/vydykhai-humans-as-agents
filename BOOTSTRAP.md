...
6. Create the dedicated organization-only Framework Orchestrator context as `[ORCHESTRATOR] <project> — Vydykhai <version>`. Read back and correct its actual title or stable handle and resolved maximum profile, pin/foreground exactly that one context when the environment supports that control, and confirm it can reconstruct compass, DOD, tasks, blockers, and next-best-action.
