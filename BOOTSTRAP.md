...
4. Create or update Project Operating Brief, atomic Project State v2, Shared Sync Contract, Project Memory Graph v3, tracker projection, and safe operational pointers. State includes one DOD Control Line, Governor receipt, Execution Leases, Pending Return Inbox, and detour/recall gates; graph includes Pending Memory Events and `CURRENT / NEXT / PRIOR_MISS` probes. Migrate old schemas side by side with preserved ids/sources, visible delta, human-confirmed cutover, and passing `control-check`; never append new current sections to old bodies.
