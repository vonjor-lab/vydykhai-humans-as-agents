...
12. Configure the marked Return Sync as durable task/tracker outbox first and native cross-context wakeup second; after consumption the orchestrator writes the paired marked Return Route receipt. Verify one real emitted id through `WRITTEN / SENT / RECEIVED / CONSUMED / ROUTED`, then prove the installed schedule returns `NOOP` with no wake, queued message, or model call. Malformed or mismatched routing must audit. Use one monitor only when neither normal route exists; never require human polling.
